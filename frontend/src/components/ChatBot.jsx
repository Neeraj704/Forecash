...
export default function ChatBot() {
  ...
  const [messages, setMessages] = useState([{ role: 'ai', content: 'Hi, how can I help you today?' }]);
  ...
  const sendMessage = async e => {
    ...
    try {
      ...
      setMessages(msgs =>
        msgs
          .filter(m => m.role !== 'ai-typing')
          .concat({ role: 'ai', content: '⚠️ Sorry, I couldn’t get a response. Try again.' })
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen(o => !o)}
        className="fixed bottom-4 right-4 w-16 h-16 bg-black rounded-full text-white"
      >
        💬
      </button>
